...
def plotSteps(startCoords, endCoords, numBreaks):
	...
	for i in range(numBreaks + 1):
		points.append([startCoords[0]+i*(endCoords[0]-startCoords[0])/numBreaks, startCoords[1]+i*(endCoords[1]-startCoords[1])/numBreaks])
	...
	for i in range(numBreaks):
		...
		for j in range(abs(steps1)):
			temp_steps[4*round(j*mostSteps/steps1)][1] = math.copysign(1, steps1)
		for j in range(abs(steps2)):
			temp_steps[1+4*round(j*mostSteps/steps2)][1] = math.copysign(1, steps2)
		for j in range(abs(steps3)):
			temp_steps[2+4*round(j*mostSteps/steps3)][1] = math.copysign(1, steps3)
		for j in range(abs(steps4)):
			temp_steps[3+4*round(j*mostSteps/steps4)][1] = math.copysign(1, steps4)
